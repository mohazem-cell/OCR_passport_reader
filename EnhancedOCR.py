...
class EnhancedOCR:
    def __init__(self, api_url: str, model: str = "unsloth/Qwen2.5-VL-72B-Instruct-bnb-4bit",language: str = "arabic"):
        self.api_url = api_url
        self.model = model
        self.language = language  # Added language support (e.g., "english", "arabic")
    ...
    def create_advanced_prompt(self, 
        ...
        language_notes = []
        reading_directions = []
        if "arabic" in [lang.lower() for lang in self.language]:
            reading_directions.append("right to left (RTL) for Arabic text")
            language_notes.append("The image may contain Arabic text. Ensure proper handling of Arabic script, diacritics, and RTL reading order where applicable.")
        if "english" in [lang.lower() for lang in self.language]:
            reading_directions.append("left to right (LTR) for English text")
            language_notes.append("The image may contain English text. Read in standard LTR order where applicable.")
        
        reading_direction = " and ".join(reading_directions) if reading_directions else "left to right (LTR)"
        language_note = " ".join(language_notes) if language_notes else f"The text is in {', '.join(self.language).capitalize()}."
        
        base_instructions = f"""You are an expert OCR system. Your task is to extract ALL text from this image with PERFECT accuracy. {language_note} Read the image carefully from top to bottom, {reading_direction}."""
               
        prompts = {
            "general": f"""{base_instructions}
                    *Instructions:*
                    1. Read the image carefully from top to bottom, {reading_direction}
